// DopaBrain Portal - Enhanced Application Logic with Pagination & Infinite Scroll
(function () {
    'use strict';
    ...
    function renderApps(apps) {
        ...
        if (currentCategory === 'all' && !searchQuery.trim()) {
            ...
            // Simple paginated grid for filtered/searched results
            const paginatedApps = regular.slice(0, itemsPerPage * currentPage);
    ...
    function initInfiniteScroll() {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting && lastObserved !== entry.target) {
                    lastObserved = entry.target;
                    const hasMore = itemsPerPage * currentPage < totalFilteredApps;
                    if (hasMore) {
